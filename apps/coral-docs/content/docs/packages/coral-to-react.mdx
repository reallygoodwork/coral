...
When using `className` format with variants, the generator automatically uses CVA if `variantStrategy` is set to `'cva'` or if variants are detected.
...
**`'inline'` (default):** Variants are applied as inline styles or CSS classes directly in the JSX.

**`'cva'`:** Uses [Class Variance Authority](https://cva.style) for variant management. Requires `styleFormat: 'className'` and Tailwind CSS. Automatically generates CVA configuration from Coral variant definitions.

**`'custom'`:** Allows custom variant handling (for advanced use cases).

#### CVA Example

When using `variantStrategy: 'cva'` with variants:
...
export const Button = (props: ButtonProps) => {
  const className = buttonVariants({ intent: props.intent, size: props.size });
...
<TypeTable
  type={{
    ...
    styleFormat: {
      description: 'How to output styles',
      type: "'inline' | 'className'",
      default: "'inline'",
    },
    includeTypes: {
    ...
    variantStrategy: {
      description: 'Variant handling strategy',
      type: "'cva' | 'inline' | 'custom'",
      default: "'inline'",
    },
  }}
