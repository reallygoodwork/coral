...
import { generateComponentTypes, generatePackageTypes } from '../../lib/generateTypes'
...
export const buildCommand = new Command('build')
  ...
  .action(async (packagePath: string, options: { target: string; outDir?: string }) => {
    ...
    try {
      ...
      const outDir =
        options.outDir ?? pkg.config.exports?.[options.target]?.outDir ?? `./dist/${options.target}`
      ...
      if (options.target === 'types') {
        ...
        console.log(`✓ Built TypeScript types for ${pkg.components.size} components to ${outDir}`)
        ...
        console.log(`✓ Built JSON files for ${pkg.components.size} components to ${outDir}`)
      } else {
